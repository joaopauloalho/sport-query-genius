...
function metricField(value: unknown, raw: JsonRecord): string | null {
  ...
  return typeof metric === "string" && FOOTBALL_METRIC_KEYS.includes(metric as never)
    ? metric
    : null;
}
...
function truthfulFilters(raw: JsonRecord): unknown[] {
  ...
  return source.map((item) => {
    ...
    const isolated = normalizeUniversalQueryPlanCandidate({
      ...raw,
      filters: [item],
      group_by: [],
    });
    const normalizedFilter = asRecord(isolated)?.filters;
    if (Array.isArray(normalizedFilter) && normalizedFilter.length === 1)
      return normalizedFilter[0];
    ...
    const field =
      canonicalMetric ??
      (fieldToken ? baseAliases[fieldToken] : null) ??
      canonicalToken(fieldValue);
    const operatorKey = token(record.operator ?? record.op);
    const operator =
      (operatorKey ? OPERATORS[operatorKey] : null) ?? canonicalToken(record.operator ?? record.op);
    return {
      field,
      operator,
...
function truthfulGroups(raw: JsonRecord): string[] {
  ...
  return items.map((item) => {
    ...
    if (Array.isArray(normalized) && normalized.length === 1 && typeof normalized[0] === "string")
      return normalized[0];
  ...
  return {
    ...
    scope: {
      ...
      ...(typeof competition === "string"
        ? { competition: canonicalizeCompetitionName(competition) }
        : {}),
    },

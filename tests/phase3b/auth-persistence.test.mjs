...
const migration = readFileSync(
  new URL("../../supabase/migrations/20260821235900_phase3b_auth_user_persistence.sql", import.meta.url),
  "utf8",
);
const envExample = readFileSync(new URL("../../.env.example", import.meta.url), "utf8");
const browserClient = readFileSync(new URL("../../src/lib/supabase-browser.ts", import.meta.url), "utf8");
const store = readFileSync(new URL("../../src/lib/store.tsx", import.meta.url), "utf8");

const userTables = ["profiles", "analysis_history", "saved_analyses", "workspaces", "workspace_items"];
...
test("every user table has ownership policies for CRUD", () => {
  for (const table of userTables) {
    for (const command of ["select", "insert", "update", "delete"]) {
      assert.match(
        migration,
        new RegExp(`create policy ${table}_[a-z_]*${command === "select" ? "select" : command}[a-z_]*[\\s\\S]*?on public\\.${table} for ${command}`, "i"),
      );
    }
  }
});
...
test("UPDATE policies enforce ownership before and after the change", () => {
  for (const table of userTables) {
    ...
    assert.match(updateBlock, new RegExp(`using \\(\\(select auth\\.uid\\(\\)\\) = ${ownerColumn}\\)`, "i"));
    assert.match(updateBlock, new RegExp(`with check \\(\\(select auth\\.uid\\(\\)\\) = ${ownerColumn}\\)`, "i"));
  }
});
...
test("authenticated user data no longer falls back to legacy localStorage", () => {
  ...
  for (const table of ["analysis_history", "saved_analyses", "workspaces", "workspace_items", "profiles"]) {
